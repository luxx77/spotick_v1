...
class HandlerManager {
  ...
  Future<void> newLogs() async {
    // for (var i in handler!.player.shuffleIndices!) {
    //   log('${handler!.player.sequence![i].tag.id}');
    // }
    log('${handler!.player.shuffleIndices!.first}');
    handler!.player.setLoopMode(LoopMode.all);
    handler!.player.shuffle();
  ...
  Future playAtIndex(int index) async {
    ...
    handler!.player.currentIndexStream.listen((event) {
      ...
      if (provider!.carouselController.ready && provider!.inCurrentSongScreen) {
        provider!.carouselController
            .animateToPage(event ?? 0, duration: Duration(milliseconds: 200));
      }
